...
const SizeGuide = ({open, setOpen}) => {
  ...
  return (
    <>
      <Modal show={open} onClose={() => setOpen(false)} size={"3xl"} popup className="rounded-lg">
            ...
            <div className="sm:flex sm:items-center sm:justify-between">
              <div>
                <div className="flex items-center gap-x-3">
                  <h1 className="text-xl font-medium text-gray-800 dark:text-white">Size Guide</h1>
                </div>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
                  All product sizes are designed to fit to the following body measurements.
                  Some products may vary from these measurements but you can still use them as a guide.
                </p>
              </div>
            </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      <tr>
                        <th scope="col"
                          ...
                          <button className="flex items-center gap-x-3 focus:outline-none">
                            <span className="font-medium text-gray-800 dark:text-white">Size</span>
                          </button>
                        </th>
                        {data[category].map((item) => (
                          <th key={item.size} scope="col"
                      ...
                      <tr>
                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                          <div>
                            <h2 className="font-medium text-gray-800 dark:text-white">Chest</h2>
                          </div>
                        </td>
                        {data[category].map((item) => (
                          <td key={`${item.size}-chest`} className="px-4 py-4 text-sm whitespace-nowrap">
                            <div>
                              <p className="text-gray-500 dark:text-gray-400">{item.chest}</p>
                            </div>
                          </td>
                        ))}
                      </tr>
                      <tr>
                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                          <div>
                            <h2 className="font-medium text-gray-800 dark:text-white">Height</h2>
                          </div>
                        </td>
                        {data[category].map((item) => (
                          <td key={`${item.size}-height`} className="px-4 py-4 text-sm whitespace-nowrap">
                            <div>
                              <p className="text-gray-500 dark:text-gray-400">{item.height}</p>
                            </div>
                          </td>
                        ))}
                      </tr>
